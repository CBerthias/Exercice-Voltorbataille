...
class GrilleCreationServiceTest {
	...
	public static Stream<Arguments> getRandomGrilleDimensions() {
		...
		return Stream.of(
				Arguments.of(random.nextInt(0, 10), random.nextInt(0, 10)),
				Arguments.of(random.nextInt(0, 10), random.nextInt(0, 10)),
				Arguments.of(random.nextInt(0, 10), random.nextInt(0, 10))
		);
	}
	...
	void testIndicesShouldHaveRightValues() {
		List<Integer> ligne1 = List.of(1, 2, 3, -1, 0, 1);
		List<Integer> ligne2 = List.of(0, 1, -1, -1, 0, 2);
		List<Integer> ligne3 = List.of(1, 2, 3, 3, 0, 1);
		List<List<Integer>> plateau = List.of(ligne1, ligne2, ligne3);

		Indice indiceLigne1 = new Indice(1, 7);
		Indice indiceLigne2 = new Indice(2, 3);
		Indice indiceLigne3 = new Indice(0, 10);

		Indice indiceColonne1 = new Indice(0, 2);
		Indice indiceColonne2 = new Indice(0, 5);
		Indice indiceColonne3 = new Indice(1, 6);
		Indice indiceColonne4 = new Indice(2, 3);
		Indice indiceColonne5 = new Indice(0, 0);
		...
		when(grilleCreationService.creerPlateau(anyInt(), anyInt(), anyInt())).thenReturn(plateau);
